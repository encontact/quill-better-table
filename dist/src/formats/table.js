...
class TableCell extends Container {
    checkMerge() {
        if (super.checkMerge() && this.next?.children.head != null) {
            ...
            return (thisHead.cell === thisTail.cell &&
...
class TableRow extends Container {
    checkMerge() {
        if (super.checkMerge() && this.next?.children.head != null) {
            ...
            return (thisHead.row === thisTail.row &&
...
class TableContainer extends Container {
    ...
    deleteColumns(compareRect, delIndexes = [], editorWrapper) {
        ...
        tableCells.forEach(cell => {
            ...
            if (cellRect.x + ERROR_LIMIT > compareRect.x &&
                cellRect.x1 - ERROR_LIMIT < compareRect.x1) {
                removedCells.push(cell);
            }
            else if (cellRect.x < compareRect.x + ERROR_LIMIT &&
    ...
    deleteRow(compareRect, editorWrapper) {
        ...
        tableCells.forEach(cell => {
            const cellRect = getRelativeRect(cell.domNode.getBoundingClientRect(), editorWrapper);
            if (cellRect.y > compareRect.y - ERROR_LIMIT &&
                cellRect.y1 < compareRect.y1 + ERROR_LIMIT) {
                removedCells.push(cell);
            }
            else if (cellRect.y < compareRect.y + ERROR_LIMIT &&
                cellRect.y1 > compareRect.y1 - ERROR_LIMIT) {
                modifiedCells.push(cell);
        ...
        const removedRowsLength = this.rows().reduce((sum, row) => {
            ...
            if (rowRect.y > compareRect.y - ERROR_LIMIT &&
                rowRect.y1 < compareRect.y1 + ERROR_LIMIT) {
                sum += 1;
            }
            return sum;
    ...
    insertColumn(compareRect, colIndex, isRight = true, editorWrapper) {
        ...
        tableCells.forEach(cell => {
            ...
            if (isRight) {
                ...
                else if (compareRect.x1 - cellRect.x > ERROR_LIMIT &&
                ...
                else if (compareRect.x - cellRect.x > ERROR_LIMIT &&
    ...
    insertRow(compareRect, isDown, editorWrapper) {
        ...
        tableCells.forEach(cell => {
            ...
            if (isDown) {
                ...
                else if (compareRect.y1 - cellRect.y > ERROR_LIMIT &&
                ...
                else if (compareRect.y - cellRect.y > ERROR_LIMIT &&
        ...
        addBelowCells.forEach(cell => {
            const cId = cellId();
            const cellFormats = cell.formats();
            const tableCell = this.scroll.create(TableCell.blotName, Object.assign({}, CELL_DEFAULT, { row: rId, colspan: cellFormats.colspan }));
            const cellLine = this.scroll.create(TableCellLine.blotName, {
                row: rId,
                cell: cId,
...
export { 
// blots
TableCol, TableColGroup, TableCellLine, TableCell, TableRow, TableBody, TableContainer, TableViewWrapper, 
// identity getters
rowId, cellId, 
// attributes
CELL_IDENTITY_KEYS, CELL_ATTRIBUTES };
