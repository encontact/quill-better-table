...
export default class TableSelection {
    ...
    helpLinesInitial() {
        ...
        LINE_POSITIONS.forEach((direction) => {
            this[direction] = this.getDivElement()(this)[direction].classList.add('qlbt-selection-line')(this)[direction].classList.add('qlbt-selection-line-' + direction);
            css(this[direction], {
                position: 'absolute',
                display: 'none',
    ...
    correctBoundary() {
        ...
        tableCells.forEach(tableCell => {
            ...
            let isCellIntersected = ((x + ERROR_LIMIT >= this.boundary.x && x + ERROR_LIMIT <= this.boundary.x1) ||
                (x - ERROR_LIMIT + width >= this.boundary.x && x - ERROR_LIMIT + width <= this.boundary.x1)) && ((y + ERROR_LIMIT >= this.boundary.y && y + ERROR_LIMIT <= this.boundary.y1) ||
    ...
    computeSelectedTds() {
        ...
        return tableCells.reduce((selectedCells, tableCell) => {
            let { x, y, width, height } = getRelativeRect(tableCell.domNode.getBoundingClientRect(), this.quill.root.parentNode);
            let isCellIncluded = (x + ERROR_LIMIT >= this.boundary.x &&
                x - ERROR_LIMIT + width <= this.boundary.x1) && (y + ERROR_LIMIT >= this.boundary.y &&
    ...
    destroy() {
        LINE_POSITIONS.forEach(direction => {
            this[direction].remove()(this)[direction] = null;
        });
