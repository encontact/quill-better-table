...
class BetterTable extends Module {
  ...
  static keyboardBindings: { 'table-cell-line backspace': { key: string; format: string[]; collapsed: boolean; offset: number; handler(range: any, context: any): boolean }; 'table-cell-line delete': { key: string; format: string[]; collapsed: boolean; suffix: RegExp; handler(): void }; 'table-cell-line enter': { key: string; shiftKey: null; format: string[]; handler(range: any, context: any): void }; 'table-cell-line up': { key: string; collapsed: boolean; format: string[]; handler(range: any, context: any): boolean }; 'table-cell-line down': { key: string; collapsed: boolean; format: string[]; handler(range: any, context: any): boolean }; 'down-to-table': { key: string; collapsed: boolean; handler(range: any, context: any): boolean }; 'up-to-table': { key: string; collapsed: boolean; handler(range: any, context: any): boolean } }
  static register() {
  ...
  constructor(quill: Quill, options: any) {
    ...
    this.quill.root.addEventListener('contextmenu', (evt) => {
      ...
      let isTargetCellSelected = this.tableSelection.selectedTds
        .map((tableCell: { domNode: any }) => tableCell.domNode)
        .includes(cellNode)
    ...
    // Include If to validade undefined situation
    if(thisBinding) quill.keyboard.bindings['Backspace'].splice(0, 1, thisBinding)
  ...
  getTable(range = this.quill.getSelection()) {
    ...
    const [cellLine, offset] = this.quill.getLine(range.index) as any[] // Must be an TableCellLine?
    if (cellLine == null || cellLine.statics.blotName !== TableCellLine.blotName) {
      return [null, null, null, -1];
    }
    const cell = cellLine.tableCell() as TableCell;
    const row = cell.row();
  ...
  showTableTools (table: any, quill: Quill, options: any) {
    this.table = table
  ...
  hideTableTools () {
    ...
    this.tableOperationMenu && this.tableOperationMenu.destroy()
    // Commented because typescript doesn't like it
...
BetterTable.keyboardBindings = {
  'table-cell-line backspace': {
    ...
    //// Original code, comment because offset is not used.
  ...
  'table-cell-line enter': {
    ...
    handler(range, context) {
      ...
      const Scope = (Quill.imports.parchment as any).Scope
      if (range.length > 0) {
        this.quill.scroll.deleteAt(range.index, range.length); // So we do not trigger text-change
      }
      const lineFormats = Object.keys(context.format).reduce((formats: any, format: any) => {
        if (
...
function isTableCell (blot: any) : any {
  return blot.statics.blotName === TableCell.blotName
}

function isInTableCell (current: any) : boolean {
